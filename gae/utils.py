...
def mask_test_edges(adj):
    ...
    train_edge_idx = all_edge_idx[num_test:]
    test_edges = edges[test_edge_idx]
...
def get_roc_score(emb, adj_orig, edges_pos, edges_neg,plot=False, save_path=None):
    def sigmoid(x):
        return 1 / (1 + np.exp(-x))

    # Predict on test set of edges
    adj_rec = np.dot(emb, emb.T)
    # Avoid self-loops by setting diagonal elements to 0
    np.fill_diagonal(adj_rec, 0)
    preds = []
    pos = []
    for e in edges_pos:
    ...
    if plot:
        ...
        if save_path:
            ...
            print(f"ROC curve saved to {os.path.join(save_path, 'roc_curve.png')}")
        else:
            plt.show()
        plt.close()
        ...
        plt.figure()
        plt.plot(recall, precision, color='purple', lw=2, label=f'PR (AP = {ap_score:.2f})')
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel('Recall')
        ...
        if save_path:
            ...
            print(f"PR curve saved to {os.path.join(save_path, 'pr_curve.png')}")
        else:
            plt.show()
        plt.close()
